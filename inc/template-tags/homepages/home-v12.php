...
if ( ! function_exists( 'electro_home_v12_slider_block' ) ) {
	...
	?><div <?php esc_attr_e(electro_render_attributes( $attr )); ?>>
		...
		<div class="row row-cols-md-3 row-cols-xl-<?= esc_attr( $categories_options[ 'categories' ][ 'columns' ] ) ?> mb-5 mb-xl-0">
			<?php foreach( $product_cats as $product_cat ) : ?>
				...
				<div class="pt-7 pt-xl-0 mb-xl-5 mt-xl-4">
					<a href="<?php esc_html_e(esc_url( $term_link )); ?>" class="categories-list-bg px-3 d-flex flex-column h-100 justify-content-end rounded-sm">
						<div class="border-bottom text-center pb-4">
							<img class="img-fluid mt-n5" src="<?php esc_html_e(esc_url( $cat_thumb_url ? $cat_thumb_url : wc_placeholder_img_src() )); ?>" alt="<?php esc_attr_e( $product_cat->name ); ?>">
						</div>
						<div class="text-center py-3 lh-lg">
							<h6 class="fw-bold mb-0" style="font-size:15px;"><?php esc_html_e( $product_cat->name ); ?></h6>
...
if ( ! function_exists( 'electro_home_v12_products' ) ) {
	...
	?><section <?php esc_attr_e(electro_render_attributes( $attr )); ?>>
		<?php if ( isset( $args[ 'section_title' ] ) && ! empty( $args[ 'section_title' ] ) ) : ?>
		<header class="mb-0">
			<h2 class="h1"><?php esc_html_e( $args[ 'section_title' ] ); ?></h2>
		</header>
	<?php endif; ?>
	<?php esc_attr_e(electro_do_shortcode( $args[ 'shortcode' ], $shortcode_atts )); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
	</section><?php
}
}
		...
		<div class="row">
			<?php foreach( $banners_options[ 'banners' ] as $key => $banner ) : ?>
				...
				<div class="<?php esc_attr_e( $column_classes ); ?>">
					<div class="d-flex align-items-center p-3 position-relative rounded-sm overflow-hidden bg-cover"<?php if( isset( $banner[ 'bg_image' ] ) && ! empty( $banner[ 'bg_image' ] ) ) : ?>style="background-image:url(<?php esc_attr_e(esc_url( $banner[ 'bg_image' ] )); ?>);"<?php endif; ?>>
						<a class="d-md-flex align-items-center stretched-link" href="<?php esc_html_e(esc_url( $banner[ 'url' ] )); ?>">
							<?php
							switch ( $i % 3 ) {
								case 1;
								if ( 'banner_1' === $key ) {
									if ( isset( $banner[ 'image' ] ) && ! empty( $banner[ 'image' ] ) ) : ?>
										<div class="media-left ms-md-n4 me-md-5 me-xxl-4 flex-shrink-0 mt-md-7 mt-xl-4 mt-xxl-7">
											<img src="<?php esc_html_e(esc_url( $banner[ 'image' ] )); ?>" alt="<?php esc_attr_e( 'Banner Image', 'electro' ); ?>">
										</div>
									<?php endif; ?>
									...
									<div class="fw-light text-dark lh-sm me-xxl-4" style="font-size:20px;letter-spacing: -.7px;">
										<?php esc_html_e(wp_kses_post( $banner[ 'desc' ] )); ?>
									</div>
								<?php endif;
							}
							break;
							...
							?><div class="px-3 d-flex lh-sm">
								<?php if ( isset( $banner[ 'desc' ] ) && ! empty( $banner[ 'desc' ] ) ) : ?>
								<div class="fw-light text-dark" style="font-size:20px;width:104px;"><?php esc_html_e(wp_kses_post( $banner[ 'desc' ] )); ?></div>
							<?php endif; ?>
							<div class="text-dark">
								<?php if ( isset( $banner[ 'price_pre_text' ] ) && ! empty( $banner[ 'price_pre_text' ] ) ) : ?>
								<div style="font-size:8px;"><?php esc_html_e( $banner[ 'price_pre_text' ] ); ?></div>
							<?php endif; ?>
							<?php if ( isset( $banner[ 'price_text' ] ) && ! empty( $banner[ 'price_text' ] ) ) : ?>
							<div class="fw-bold" style="font-size:30px;">
								<?php esc_html_e(wp_kses_post( $banner[ 'price_text' ] )); ?>
							</div>
						<?php endif; ?>
					</div>
					</div><?php
				...
				?><div class="px-3 lh-1">
					<div class="text-dark" style="width:138px;">
						...
						<h6 class="mb-0"><?php esc_html_e( $banner[ 'title' ] ); ?></h6>
					<?php endif; ?>
					<?php if ( isset( $banner[ 'subtitle' ] ) && ! empty( $banner[ 'subtitle' ] ) ) : ?>
					<h4 class="mb-0 h3 fw-light"><?php esc_html_e( $banner[ 'subtitle' ] ); ?></h4>
				<?php endif; ?>
				<?php if ( isset( $banner[ 'desc' ] ) && ! empty( $banner[ 'desc' ] ) ) : ?>
				<span class="fw-bold d-block lh-sm" style="font-size:12px;"><?php esc_html_e( $banner[ 'desc' ] ); ?></span>
			<?php endif; ?>
		</div>
		</div><?php
	}
...
if ( ! function_exists( 'electro_home_v12_blog_posts' ) ) {
	...
	?><section <?php esc_attr_e(electro_render_attributes( $attr )); ?>>
		...
		<header class="mb-3">
			<h2 class="h1"><?php esc_html_e( $blog_posts_options[ 'section_title' ] ); ?></h2>
		</header>
	<?php endif; ?>
	<?php if ( $posts_query->have_posts() ) : ?>
		<div class="row row-cols-lg-<?php esc_attr_e( $blog_posts_options[ 'columns' ] ); ?> gy-3">
					...
					<?php if ( has_post_thumbnail() ) : ?>
						...
						<div class="rounded-sm overflow-hidden">
							<a href="<?php the_permalink(); ?>" class="d-block">
								<img class="img-fluid w-100" src="<?php esc_html_e(esc_url( $post_thumnail_url )); ?>" alt="<?php esc_attr_e( 'Post Image', 'electro' ); ?>" style="height285px;object-fit: cover;">
							</a>
						</div>
					<?php endif; ?>
					<?php
					...
					<div class="px-md-3 px-2 pt-4 pb-3">
						<?php if ( $post_categories ) : ?>
							<?php foreach( $post_categories as $key => $category ) :?>
								<a href="<?php esc_html_e(esc_url( get_category_link( $category->term_id ) )); ?>" class="mb-3 fw-bold d-inline-block"><?php esc_html_e( $category->name ); ?></a>
								<?php if( count( $post_categories ) !== $key + 1 ) : ?>
									<?php esc_html_e( $separator ); ?>
								<?php endif; ?>
							<?php endforeach; ?>
						<?php endif; ?>
						...
						<?php if ( has_excerpt() ) : ?>
							<p class="mb-0" style="color:#878787;letter-spacing: .1px;"><?php esc_html_e(wp_kses_post( get_the_excerpt() )); ?></p>
						<?php endif; ?>
					</div>
				</div>
...
if ( ! function_exists( 'electro_home_v12_brands_block' ) ) {
	...
	?><section <?php esc_attr_e(electro_render_attributes( $attr )); ?>>
		<?php if ( ! empty( $brands_options[ 'section_title' ] ) ) : ?>
			<header class="mb-4">
				<h2 class="h1"><?php esc_html_e( $brands_options[ 'section_title' ] ); ?></h2>
			</header>
		<?php endif; ?>
		<div class="row row-cols-md-2 row-cols-lg-<?php esc_attr_e( $brands_options[ 'columns' ] ); ?> gy-6 pt-2">
			<?php foreach ( $terms as $term ) :	?>
				<div class="brand-card-list">
					<?php if ( $brand->public ) : ?>
						<a href="<?php esc_html_e(esc_url( apply_filters( 'ec_brand_link', get_term_link( $term ), $term ) )); ?>" class="mme-xl-4 rounded-sm px-4 py-4 position-relative h-100 d-block">
						<?php else: ?>
							...
							?>
							<img class="img-fluid mb-3" src="<?php esc_html_e(esc_url( $image_src )); ?>" alt="<?php esc_attr_e( $term->name ); ?>" width="<?php esc_attr_e( $image_width ); ?>" height="<?php esc_attr_e( $image_height ); ?>">
							<?php if ( $term->description ) : ?>
								<p class="me-xl-4 me-xxl-7" style="color:#878787;letter-spacing: .1px;"><?php esc_html_e( $term->description ); ?></p>
							<?php endif; ?>
							<?php if ( $cover_image_url && ! empty( $cover_image_url ) ) : ?>
								<img class="img-fluid ms-n4 ms-xl-n2 ms-xxl-n4 mt-n2 mt-lg-n4 position-absolute top-100 start-100 translate-middle" src="<?php esc_html_e(esc_url( $cover_image_url )); ?>" alt="<?php esc_attr_e( 'Attribute Image', 'electro' ); ?>">
							<?php endif; ?>
							<?php if ( $brand->public ) : ?>
							</a>
						<?php else: ?>
