	...
	function electro_template_single_brand() {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			foreach ( $terms as $term ) {
				...
				$value = get_field( "product_attribute_cover_image", $term );
				if ( $thumbnail_id ) {
				...
				$brand_img 	.= '<a href="' . esc_url( get_term_link( $term ) ). '"><img src="' . esc_url( $value ) . '" alt="' . esc_attr( $term->name ) . '" /></a>';
			}
		}
...
if( ! function_exists( 'electro_single_product_deal_countdown_timer' ) ) {
	function electro_single_product_deal_countdown_timer() {
